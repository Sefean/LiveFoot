...
export default function SelectPlayerSubstitutionOut({route, navigation}) {
    
    const params = route.params;
    
    const jugadores = params.jugadores;
    const partido = params.partido;
    const jugadorEntraId = params.jugadorEntraId;
    const jugadorEntraNombre = params.jugadorEntraNombre;

    const [comentario, setComentario] = useState("Entra al campo " + jugadorEntraNombre);
    const [jugadorSale, setJugadorSale] = useState(0);

    const addJugadorToComment = (jugador) =>
    {
        setComentario("Entra al campo " + jugadorEntraNombre + " y sale " + jugador.nombre);
        setJugadorSale(jugador.id_jugador);
    }
    ...
    const sendComment = () =>
    {
        ...
        let data = {
            ...
            id_jugador1: jugadorEntraId,
            id_jugador2: jugadorSale
        };
